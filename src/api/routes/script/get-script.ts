...
export default function (app: FastifyInstance, logger: ILogger, appConfig: AppConfig) {
    app.get('/api/:repo/script/:id', {
        schema: <FastifySchema>{
            description: "get a script for the given user, workspace, and repo",
            tags: ['api', 'repo'],
            summary: "gets an script.",
            ...
            response: {
                200: readJsonFile(appConfig.schemaRepoPath, "yaitde-script.json")
            }
        }
    },
        async (req, reply) => {
            try {
                ...
                const model = await yaitdeRepo.getScript(id);
                reply.code(200)
